...
def preprocess(batch):
    ...
    images = images / jnp.linalg.norm(images, axis=-1)[:, None]
    labels = batch['label']
...
best_acc = 0
Path('ckpt/mnist_multi').mkdir(parents=True, exist_ok=True)
...
for step in range(NUM_ITER):
    ...
    if (step + 1) % CHECK_FREQ == 0:
        ...
        if valid_acc > best_acc:
            best_acc = valid_acc
            save_checkpoint('ckpt/mnist_multi', params, step=valid_acc, overwrite=True)
    bar.update()
...
Path('img/mnist_multi').mkdir(parents=True, exist_ok=True)

plt.plot(loss_list)
plt.xlabel('iteration')
plt.ylabel('loss')
plt.grid()
plt.savefig('img/mnist_multi/train_loss.png')
plt.close()
...
plt.ylim(0, 1)
plt.grid()
plt.savefig('img/mnist_multi/valid_acc.png')
plt.close()
