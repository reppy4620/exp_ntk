...
train_ds = create_ds(train_x, train_y).batch(1000).cache().prefetch(tf.data.AUTOTUNE).repeat().shuffle(1024).as_numpy_iterator()
...
def main(mid_channel):
    ...
    init_fn, apply_fn, kernel_fn = stax.serial(
        stax.Dense(mid_channel, parameterization='standard', b_std=1),
        stax.Relu(),
        stax.Dense(NUM_CLASSES, parameterization='standard', b_std=1)
    )
