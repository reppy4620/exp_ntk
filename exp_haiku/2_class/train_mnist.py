...
valid_ds = tfds.as_numpy(valid_ds.map(preprocess_data).batch(BATCH_SIZE))
...
net = hk.without_apply_rng(hk.transform(mlp))
dummy_input = next(train_ds)
dummy_image, _= preprocess(dummy_input)
params = net.init(jax.random.PRNGKey(SEED), dummy_image)
